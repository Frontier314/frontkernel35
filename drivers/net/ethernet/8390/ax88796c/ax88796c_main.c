	...
	while (!(AX_READ (ax_base + PG_PSR) & PSR_DEV_READY))
	{
		if (time_after(start, start + 2 * HZ / 100)) {		/* 20ms */
			printk (KERN_ERR 
				...
				ax_local->ndev->name);
printk ("%s: timeout waiting for reset completion\n",	ax_local->ndev->name);			return -1;
		}
	}
...
unsigned char ethaddr[64];
static int __init param_ethaddr_setup(char *str)
{
  strcpy(ethaddr,str);
  return 0;
}
__setup("ethaddr=", param_ethaddr_setup);
//fighter--
/*
 * ----------------------------------------------------------------------------
...
//fighter++
		if(strlen(ethaddr) == 17){
			ndev->dev_addr[0] = (char2hex(ethaddr[0])&0xf)<<4  | (char2hex(ethaddr[1])&0xf);
			ndev->dev_addr[1] = (char2hex(ethaddr[3])&0xf)<<4  | (char2hex(ethaddr[4])&0xf);
			ndev->dev_addr[2] = (char2hex(ethaddr[6])&0xf)<<4  | (char2hex(ethaddr[7])&0xf);
			ndev->dev_addr[3] = (char2hex(ethaddr[9])&0xf)<<4  | (char2hex(ethaddr[10])&0xf);
			ndev->dev_addr[4] = (char2hex(ethaddr[12])&0xf)<<4 | (char2hex(ethaddr[13])&0xf);
			ndev->dev_addr[5] = (char2hex(ethaddr[15])&0xf)<<4 | (char2hex(ethaddr[16])&0xf);
	  }else{
...
#else
	unsigned long irq_flag = IRQF_SHARED | IORESOURCE_IRQ_LOWLEVEL ;
#endif
